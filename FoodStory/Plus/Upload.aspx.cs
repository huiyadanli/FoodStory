...
public partial class Upload : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (user.FacePath == "")
        {
            ...
            fileName = DateTime.Now.ToString("yyyyMMddhhmmssff") + CreateRandomCode(8);
        }
        else
        {
            fileName = user.FacePath;
        }
        //定义一个变量用以储存当前头像的序号
        int avatarNumber = 1;
        //遍历所有文件域
        foreach (string fieldName in Request.Files.AllKeys)
        {
            ...
            if (fieldName == "__source")
            {
                ...
                result.sourceUrl = string.Format("~/Static/Image/Face/face_source_{0}.jpg", fileName);
                //file.SaveAs(Server.MapPath(result.sourceUrl));  //不储存原图
            ...
            {
                string virtualPath = string.Format("~/Static/Image/Face/face_avatar{0}_{1}.jpg", avatarNumber, fileName);
        ...
        if (user.FacePath == "")
        {
            userBLL.ModifyFace(Session["userName"].ToString(), fileName);
        }
